...
module.exports = {
  async execute(interaction) {
    ...
    if (interaction.customId === 'sales_report_modal') {
      ...
      const filePath = `data/sales_reports/${guildId}/uriage-houkoku-${normalizedDate}-${username}.json`;
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const logPath = `logs/sales_reports/${guildId}/uriage-houkoku-${normalizedDate}-${username}_${timestamp}.json`;
    ...
    if (interaction.customId.startsWith('edit_sales_report_modal_')) {
      ...
      const newButtons = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          ...
          .setStyle(ButtonStyle.Success),
        new ButtonBuilder()
          .setCustomId(`edit_report_${normalizedDate}_${interaction.user.id}`) // 新しい日付を使用
          .setLabel('今の報告を修正')
          .setStyle(ButtonStyle.Primary)
      );
      ...
      const originalFilePath = `data/sales_reports/${guildId}/uriage-houkoku-${originalDate}-${username}.json`;
      const newFilePath = `data/sales_reports/${guildId}/uriage-houkoku-${normalizedDate}-${username}.json`;
